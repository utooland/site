...
- [Package Specs](/en/docs/utoo/specs) — full list of dependency specifier forms (`npm:`, `workspace:`, `catalog:`, `file:`, `http(s):`, git, github:), and how versions are normalized when added.
- [Node Runtime](/en/docs/utoo/runtime) — declaring `engines.install-node` in `package.json` makes `ut install` install a matching Node binary alongside dependencies.
- [Migrating from pnpm](/en/docs/utoo/migration) — details on `--from pnpm`.
...
Re-runs the dependency install hooks (`preinstall`, `install`, `postinstall`) for every package in `node_modules`. See [Lifecycle Scripts](/en/docs/utoo/lifecycle) for the full list of hooks and the script environment.
...
`supports-semver` reports whether the registry supports server-side version-range queries (the `/pkg/^range` URL form). See [Registry Acceleration](/en/docs/utoo/registry-acceleration#semver-query-acceleration) for what that unlocks and [Configuration](/en/docs/utoo/config#arrays) for the on-disk cache format.
