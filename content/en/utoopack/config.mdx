...
`@utoo/pack` can be configured with `utoopack.json`, `utoopack.config.mjs`, `utoopack.config.js`, or the Node API.
...
`target` is a [Browserslist](https://browsersl.ist) query. Omit it for a browser build. When the first resolved distribution is `node`, for example `"node 20"` or the shorthand `"node"`, the build targets the Node.js platform.
...
`copy` accepts source path strings or `{ "from": "./public", "to": "static" }` objects; `to` defaults to the source path.

In production, client JS and CSS chunks default to short content-hashed names using Turbopack's 13-character hash, while development builds keep readable names. Explicit `output.filename`, `output.chunkFilename`, and `output.cssFilename` templates take precedence; use `"filename": "[name].js"` when consumers need stable entry filenames.
...
`styles.postcss` runs after the plugins from a discovered `postcss.config.*` file in the same PostCSS pass. Set `styles.postcss.implementation` to an absolute path such as `require.resolve("postcss")` to pin the PostCSS module used at build time.
...
`reactCompiler` accepts a boolean or an object with `compilationMode` and `target` for the Rust React Compiler. `compilationMode` supports `infer`, `annotation`, and `all`; `target` supports React `17`, `18`, and `19` and defaults to `19`. React `17` and `18` require `react-compiler-runtime`.
...
| `inferModuleSideEffects` | `boolean` | Infer side-effect-free modules from source when package metadata does not declare `sideEffects`; defaults to `true` |
...
| `concatenateModules` | `boolean` | Concatenate modules to reduce chunk count; the CLI enables it by default in production |
...
Utoopack's built-in list is maintained with reference to Next.js, but the [`DEFAULT_OPTIMIZE_PACKAGE_IMPORTS`](https://github.com/utooland/utoo/blob/next/packages/pack/src/core/project.ts#L107-L182) implementation is the source of truth. It currently contains 74 packages or package subpaths:
...
`devServer` supports `hot`, `dynamicHmrChunkLists`, `host`, `port`, `https`, `browserToTerminal`, and `proxy`.
...
`browserToTerminal` forwards browser console output to the development terminal: `"error"` forwards errors, `"warn"` also forwards warnings, and `true` forwards all supported console output. It defaults to `false`.

Each `proxy` rule supports:

| Field | Description |
|-------|-------------|
| `context` | Path prefix or prefixes to match; a value starting with `^` is treated as a regular expression |
| `target` | Target server URL |
| `pathRewrite` | Object of regex-to-replacement pairs where the first match wins, or a `(path) => string` function in JS configs |
| `changeOrigin` | Rewrite the `Host` header to match the target; defaults to `true` |
| `secure` | Verify the target's TLS certificate; set `false` to accept self-signed certificates |

### server

`server` configures server entries, Server Functions boundaries, server chunk output, and server-only `resolve` / `externals` overrides. `entry` accepts a single entry string or an array of named entries. The first item in an entry array is the primary server runtime and receives Server Functions.
...
`server.resolve` is applied only to server builds: its `alias` entries override matching top-level `resolve.alias` entries, and its `extensions` replace `resolve.extensions` when set. `server.externals` replaces the top-level `externals` map for server entries and Server Functions when provided; omit it to keep using top-level `externals`, or set it to `{}` to disable externals for server builds.
...
| `tracing` | `boolean` | Show compilation progress; enabled by default. Rust tracing logs are controlled by `RUST_LOG` |
...
`build(options, projectPath?, rootPath?)` and `serve(options, projectPath?, rootPath?, serverOptions?)` accept either `BundleOptions`, which is the `config` object plus runtime fields such as `processEnv`, `watch`, `dev`, `buildId`, `tracing`, and `packPath`, or a `WebpackConfig` in compatibility mode. When `rootPath` is omitted it is discovered from the project path. `serverOptions` for `serve()` supports `port`, `hostname`, `https`, `logServerInfo`, `selfSignedCertificate`, and an `onReady` callback that fires once the first build is written and the server is listening.
