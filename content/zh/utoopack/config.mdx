...
`@utoo/pack` 支持通过 `utoopack.json`、`utoopack.config.mjs`、`utoopack.config.js` 和 Node API 进行配置。
...
`target` 是一个 [Browserslist](https://browsersl.ist) 查询。不设置时按浏览器构建；当解析出的第一个目标是 `node`（例如 `"node 20"` 或简写 `"node"`）时，会按 Node.js 平台构建。
...
`copy` 支持源路径字符串，也支持 `{ "from": "./public", "to": "static" }` 对象；`to` 默认与源路径相同。

生产环境下，客户端 JS 和 CSS chunk 默认使用 Turbopack 的 13 位内容哈希短文件名，开发环境保留可读文件名。显式设置的 `output.filename`、`output.chunkFilename` 和 `output.cssFilename` 模板优先；需要稳定入口文件名时可使用 `"filename": "[name].js"`。
...
`styles.postcss` 中的插件会在发现的 `postcss.config.*` 之后、同一轮 PostCSS 处理中执行。可以通过 `styles.postcss.implementation` 指定构建时使用的 PostCSS 模块绝对路径（例如 `require.resolve("postcss")`）。
...
`reactCompiler` 可以是布尔值，也可以通过 `compilationMode` 和 `target` 配置 Rust React Compiler。`compilationMode` 支持 `infer`、`annotation` 和 `all`，`target` 支持 React `17`、`18` 和 `19`，默认 `19`；React `17` 和 `18` 需要安装 `react-compiler-runtime`。
...
| `inferModuleSideEffects` | `boolean` | 当包元数据未声明 `sideEffects` 时，从源码推断无副作用模块；默认开启 |
...
| `concatenateModules` | `boolean` | 合并模块以减少 chunk 数量；CLI 在生产环境默认开启 |
...
Utoopack 的内置列表参考 Next.js 维护，但以 [`DEFAULT_OPTIMIZE_PACKAGE_IMPORTS`](https://github.com/utooland/utoo/blob/next/packages/pack/src/core/project.ts#L107-L182) 的实现为准。当前共包含 74 个包或包子路径：
...
`devServer` 支持 `hot`、`dynamicHmrChunkLists`、`host`、`port`、`https`、`browserToTerminal` 和 `proxy`。
...
`browserToTerminal` 会把浏览器控制台输出转发到开发终端：`"error"` 只转发错误，`"warn"` 同时转发警告，`true` 转发所有支持的控制台输出。默认关闭。

每条 `proxy` 规则支持以下字段：

| 字段 | 描述 |
|------|------|
| `context` | 需要代理的路径前缀，可以是数组；以 `^` 开头时按正则匹配 |
| `target` | 目标服务地址 |
| `pathRewrite` | 正则到替换串的对象，命中第一条即生效；JS 配置中也可以传 `(path) => string` 函数 |
| `changeOrigin` | 是否把 `Host` 头改写为目标地址，默认 `true` |
| `secure` | 是否校验目标的 TLS 证书；自签名证书可设为 `false` |

### server

`server` 配置服务端入口、Server Functions 边界、服务端 chunk 输出，以及服务端专用的 `resolve` / `externals` 覆盖。`entry` 可以是单个入口字符串，也可以是具名入口数组；数组中的第一个入口是主服务端运行时，并接收 Server Functions。
...
`server.resolve` 只作用于服务端构建：其中的 `alias` 会覆盖同名的顶层 `resolve.alias`，`extensions` 设置后会整体替换 `resolve.extensions`。`server.externals` 设置后会替换服务端入口和 Server Functions 使用的顶层 `externals`；不设置时继续沿用顶层 `externals`，设为 `{}` 可以让服务端构建不使用任何 externals。
...
| `tracing` | `boolean` | 是否显示编译进度，默认开启；Rust tracing 日志由 `RUST_LOG` 环境变量控制 |
...
`build(options, projectPath?, rootPath?)` 和 `serve(options, projectPath?, rootPath?, serverOptions?)` 接收 `BundleOptions`（即 `config` 对象加上 `processEnv`、`watch`、`dev`、`buildId`、`tracing`、`packPath` 等运行时字段），或者兼容模式下的 `WebpackConfig`。不传 `rootPath` 时会从项目路径自动向上查找。`serve()` 的 `serverOptions` 支持 `port`、`hostname`、`https`、`logServerInfo`、`selfSignedCertificate`，以及在首次构建写出、服务开始监听后触发的 `onReady` 回调。
