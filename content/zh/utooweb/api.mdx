...
| `workerUrl` | string | Specifies the URL of the Worker thread where the `Project` instance's core logic actually runs. The `Project` object you interact with in the main thread is a proxy that delegates all core tasks (like file system operations) to this Worker. |
| `threadWorkerUrl` | string | **Required.** Specifies the URL of a separate Worker thread dedicated to handling CPU-intensive tasks like bundling and compiling. |
| `loaderWorkerUrl` | string | **Required.** Specifies the URL of a separate Worker thread dedicated to handling webpack loaders. |
| `serviceWorker` | object | Service worker configuration. See sub-options below. |
| `loadersImportMap` | object | A map for configuring Webpack loader imports. Keys are loader names, values are UMD/CommonJS module URLs or content strings. Loaders will be executed in parallel in a web worker pool. |
...
| `encoding` | string | The encoding of the file (e.g., `'utf8'`). If not provided, it returns a Buffer. |
...
| `options.recursive` | boolean | If `true`, performs a recursive directory removal. Defaults to `false`. |
...
**Returns:** `Promise<Stats>` - Stats object with `size`, `isDirectory()`, `isFile()`, timestamps.
...
| Option | Type | Description |
|--------|------|-------------|
| `registry` | string | The npm registry URL. Defaults to `https://registry.npmmirror.com`. |
| `concurrency` | number | Maximum concurrent network requests. Defaults to `20`. |
...
| Parameter | Type | Description |
|-----------|------|-------------|
| `onUpdate` | function | Callback when a build completes, receives the build result. Optional. |
...
| `origin` | string | The origin of the preview page (defaults to `*`). Optional. |
