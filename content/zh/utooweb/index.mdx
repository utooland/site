...
| `workerUrl` | string | Specifies the URL of the Worker thread where the `Project` instance's core logic actually runs. |
| `threadWorkerUrl` | string | **Required.** URL of a separate Worker thread for CPU-intensive tasks like bundling and compiling. |
| `loaderWorkerUrl` | string | **Required.** URL of a separate Worker thread for handling webpack loaders. |
