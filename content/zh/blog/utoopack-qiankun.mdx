...
Utoo<sup>[1]</sup> 是蚂蚁集团开源的一套基于 Rust 的前端工具链，目前主要包含包管理器和构建工具两部分。

其中，Utoopack 是 Utoo 工具链中负责前端构建的工具，底层基于 Next.js 背后的 Rust Bundler Turbopack<sup>[2]</sup>。

qiankun<sup>[3]</sup> 是蚂蚁集团开源的微前端框架，底层基于 single-spa。
...
Umi<sup>[4]</sup> 是蚂蚁中后台前端框架的开源版本。它的 qiankun 插件不再要求 Utoopack 子应用输出 UMD，而是通过浏览器全局变量暴露生命周期。

后续适配又补齐了 qiankun 2 的异步生命周期代理和端到端测试，相关实现可以参考 Umi PR #13178<sup>[5]</sup> 和 PR #13354<sup>[6]</sup>。

evjs<sup>[7]</sup> 是蚂蚁全栈框架的开源版本。它提供了独立的 `@evjs/plugin-qiankun`，支持 qiankun 主应用和子应用接入，并针对 qiankun 3 的 HTML Entry、挂载路径和生命周期桥进行了适配。

在 Utoopack 子应用中，evjs 同样通过全局变量和 Promise Proxy 与 qiankun 对接，相关实现可以参考 evjs PR #35<sup>[8]</sup> 和 PR #44<sup>[9]</sup>。
...
Turbopack PR #77773 将 Next.js 客户端 Chunk 从路径字面量切换到了 `document.currentScript`<sup>[10]</sup>；PR #78545 则直接记录并修复了 `StringLiteral` 与 Content Hash 组合导致的无限循环<sup>[11]</sup>。
...
相关实现可以参考 Utoo Issue #2526<sup>[12]</sup> 和 Utoo PR #2528<sup>[13]</sup>。
...
相关实现可以参考 Utoo PR #2790<sup>[14]</sup>。

在 Umi<sup>[4]</sup> 的 qiankun 插件中，子应用 HTML 会提前写入：
...
除此之外，Utoopack 还支持 `output.publicPath: "auto"`，根据当前执行脚本的 URL 自动计算资源路径，相关实现可以参考 Utoo PR #2953<sup>[15]</sup>。
...
相关修复可以参考 qiankun PR #3131<sup>[16]</sup>。
...
Utoopack 早期也实现过类似能力，通过 `output.entryRootExport` 将入口模块导出包装到指定的全局变量中，相关实现可以参考 Utoo PR #2550<sup>[17]</sup>。
...
这也是目前 Umi + Utoopack 微前端集成的基础，相关实现可以参考 Umi PR #13178<sup>[5]</sup>。
...
相关实现和主子应用 E2E 用例可以参考 Umi PR #13354<sup>[6]</sup>。
...
Umi PR #13178<sup>[5]</sup> 是 Umi 侧最早的一层适配。
...
Umi PR #13354<sup>[6]</sup> 在前一个 PR 的全局桥之上，补齐了 qiankun 2 的异步时序。
...
Umi PR #13367<sup>[18]</sup> 是对上述 E2E 的跨平台补充。
...
本文介绍的现有实现仍然以经典脚本模式为主。qiankun 3 ESM Sandbox 的设计可以参考相关 RFC<sup>[19]</sup>。
...
至此，Runtime 隔离才真正覆盖生产环境和开发环境，相关实现可以参考 Utoo PR #3251<sup>[20]</sup>。
...
这个修改看起来不大，却会直接决定微前端本地开发是否稳定。相关实现同样包含在 Umi PR #13354<sup>[6]</sup> 中。
...
验证通过后，笔者将这项能力下沉到 Turbopack，并通过 vercel/next.js PR #88790<sup>[21]</sup> 为 Browser Chunking Context 增加可配置的 `chunk_loading_global`。
...
在这个底层能力之上，后续的 vercel/next.js PR #93488<sup>[22]</sup> 又将它暴露为 Next.js 配置：
...
笔者先在 Utoo 的 Next.js 分支通过 utooland/next.js PR #168<sup>[23]</sup> 完成底层修改，再通过 utooland/utoo PR #3251<sup>[20]</sup> 接入 Utoopack。

在 Utoopack 中，我们使用主应用和子应用同时运行的方式完成浏览器验证，确认两个 Runtime 可以创建独立 Listener Provider 和 HMR WebSocket。

验证通过后，笔者继续向 Next.js 上游提交了 vercel/next.js PR #95997<sup>[24]</sup>，让 HMR Listener 从 Chunk Loading Global 派生：
...
[2] Turbopack（Next.js 仓库）：https://github.com/vercel/next.js/tree/canary/turbopack
...
[8] evjs PR #35：新增 `@evjs/plugin-qiankun` 主/子应用桥接插件：https://github.com/afx-team/evjs/pull/35

[9] evjs PR #44：Utoopack 子应用通过全局生命周期和 Promise Proxy 接入 qiankun：https://github.com/afx-team/evjs/pull/44

[10] Turbopack PR #77773：使用 `document.currentScript` 替代 Chunk 路径字面量：https://github.com/vercel/next.js/pull/77773

[11] Turbopack PR #78545：修复 Content Hash 与 `StringLiteral` 组合导致的构建挂起：https://github.com/vercel/next.js/pull/78545
...
[21] Turbopack 支持 `chunk_loading_global`：https://github.com/vercel/next.js/pull/88790

[22] Next.js 支持 `turbopack.chunkLoadingGlobal`：https://github.com/vercel/next.js/pull/93488

[23] Utoo Next.js 分支支持 HMR Listener 隔离：https://github.com/utooland/next.js/pull/168

[24] Turbopack HMR Listener 隔离：https://github.com/vercel/next.js/pull/95997
